...
public interface MovieDAO {
    ...
    public String QUERY_GET_MOVIE_BY_NAME ="  SELECT m.movieId ,m.movieName, m.noOfTickets, m.theatreId, m.image, t.theatreName \r\n"
     		+ "    FROM movie m  JOIN theatre t ON m.theatreId = t.theatreId WHERE movieName LIKE CONCAT('%',?,'%')";    		
//     		+ "    FROM movie m  JOIN theatreS t ON m.theatreId = t.theatreId WHERE movieName= ? ";
    ...
    public String QUERY_GET_MOVIE_BY_THEATRE_NAME = "SELECT m.movieId ,m.movieName, m.noOfTickets, m.theatreId, m.image, t.theatreName \r\n"
            + "    FROM movie m  JOIN theatre t ON m.theatreId = t.theatreId WHERE theatreName LIKE CONCAT('%',?,'%')";
//          + "    FROM movie m  JOIN theatreS t ON m.theatreId = t.theatreId WHERE theatreName= ? lower(column) LIKE lower('%value%'); ";  
    
    public String QUERY_GET_MOVIES_BY_CATEGORY_NAME = "SELECT m.movieId, m.movieName, m.category, m.noOfTickets, m.theatreId, m.image, t.theatreName FROM movie m  JOIN theatre t ON m.theatreId = t.theatreId WHERE category = ? ";
    ...
    public List<Movie> getAllMovieByCategory(String categoryname); 
