...
public class UserController {
	...
	@PutMapping("/user")
	public MyUser modifyUser(@RequestBody MyUser user)
	{
	...
	public Optional<MyUser> login(@PathVariable String username, @PathVariable String password,@PathVariable String role )
	{
	
		Optional<MyUser> user = ms.read(username);
		System.out.println("logging --> " + user);
		if(user.isPresent()) {
			if(user.get().getPassword().equals(password))
				return user;
			else {
				return Optional.ofNullable(new MyUser("Incorrect Password"));
			}
		}else {
			return null;
		}
